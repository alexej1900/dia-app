...
- **No unit test for `ErrorBoundary`.** This codebase has no React Native rendering test environment (`testEnvironment: "node"`, no `jest-expo`/`@testing-library/react-native`/`react-test-renderer`), and confirmed empirically during planning: even a bare `import { View } from 'react-native'` inside a Jest test fails under this config (`SyntaxError: Cannot use import statement outside a module`, from RN's Flow-typed `index.js`) since nothing transforms `node_modules/react-native`. Adding that infrastructure is out of scope for this module. `ErrorBoundary`'s only logic (`getDerivedStateFromError` unconditionally returning `{ hasError: true }`) is also too trivial to be worth extracting into a separate pure file just to have something to test — that would be a test asserting a constant, not real behavior. This matches the established, repeatedly-applied pattern in this codebase: components/screens get zero unit-test coverage; only genuinely branching pure logic does. `ErrorBoundary` is verified by `tsc --noEmit` and, optionally, a manual spot-check (temporarily throw in a child to confirm the fallback renders, then revert) the same way prior modules spot-checked screen behavior.
