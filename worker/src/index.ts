...
import {
  buildAnthropicRequestParams as buildTranslateNameRequestParams,
  parseAnthropicToolResult as parseTranslateNameToolResult,
...
async function handleTranslateName(request: Request, env: Env): Promise<Response> {
  let body;
  try {
    body = parseTranslateNameRequestBody(await request.json());
  } catch (e) {
    const status = e instanceof TranslateNameRequestError ? e.status : 400;
    return jsonResponse({ error: e instanceof Error ? e.message : 'Invalid request' }, status);
  }
  ...
  let message;
  try {
    message = await client.messages.create(buildTranslateNameRequestParams(body));
  } catch (e) {
    console.error('Anthropic call failed:', e instanceof Error ? e.message : e, e instanceof Anthropic.APIError ? { status: e.status, name: e.name } : undefined);
    return jsonResponse({ error: 'Translation request failed' }, 502);
  }
  ...
  try {
    const result = parseTranslateNameToolResult(toolUse.input);
    return jsonResponse(result, 200);
  } catch (e) {
    const status = e instanceof TranslateNameRequestError ? e.status : 502;
    return jsonResponse({ error: e instanceof Error ? e.message : 'Invalid model response' }, status);
  }
}
...
const ROUTES: Record<string, ((request: Request, env: Env) => Promise<Response>) | undefined> = {
  ...
  '/translate-name': handleTranslateName,
};
